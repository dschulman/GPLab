...
# ╔═╡ b6bd6c7e-e373-11ed-14ab-1f81c5f32710
using RDatasets

# ╔═╡ 5d06e1f6-1daf-4137-9574-311380d3745f
using DataFrames

# ╔═╡ 51afe74a-6409-4e24-99b9-8d7f346c0cf4
using CairoMakie

# ╔═╡ 3fcb3fc7-a4b1-4ca9-b05f-919c013251a2
mcycle_raw = dataset("MASS", "mcycle")

# ╔═╡ 29466dd6-87e8-401a-a7ac-625c8a20b167
begin
	mcycle = combine(groupby(mcycle_raw, :Times)) do sdf
		(Accel=(sdf[:, :Accel],),)
	end
	mcycle[!, :Accel] = first.(mcycle[:, :Accel])
	mcycle
end

# ╔═╡ 654df32c-0c1b-44a1-b423-2337a351800a
scatter(mcycle_raw[:, :Times], mcycle_raw[:, :Accel];
	axis = (; xlabel="Time", ylabel="Accel")
)

# ╔═╡ 46f64ded-82f9-4dce-ada9-3913fcee3488
flatten(mcycle, :Accel)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "369eaf5e9e2445b3cb6b71284f86edec40d45624"

...
# ╔═╡ f92dd3c0-e37b-11ed-204d-95653e7147a4
using RDatasets

# ╔═╡ 04f9a91e-d571-41bc-b66f-c70c4ce54895
using CairoMakie

# ╔═╡ c0b2fe67-9ecf-4fb5-95af-6e6c3bb5c633
using AbstractGPs

# ╔═╡ e5246586-bd02-44ab-96c8-4eabe825ab62
using ParameterHandling

# ╔═╡ 7ec5ca85-05f7-45cd-9d8d-9ce12ac59f6b
using Optim

# ╔═╡ ed31e444-7eb7-4ff4-9e32-4bccd53d6f08
using LineSearches

# ╔═╡ 3b9cc5f6-66f5-46db-8887-706ac405fdcd
using Zygote

# ╔═╡ 54fd6fa7-900e-41b7-8f98-931168e481c4
using BenchmarkTools

# ╔═╡ be82f8d9-a0d4-490b-a116-d8edf9c41ac6
mcycle = dataset("MASS", "mcycle")

# ╔═╡ 5569074b-f3c3-49a0-a4d4-8715cbc95e2b
scatter(mcycle[:,:Times], mcycle[:,:Accel];
	axis = (; xlabel="Time", ylabel="Accel")
)

# ╔═╡ caa4c2ae-9f89-44dd-bb46-95dd58476c52
f = GP(10.0 * Matern52Kernel() ∘ ScaleTransform(0.1))

# ╔═╡ 24da3f71-44d5-472a-9595-09c7894c293e
x = mcycle[:,:Times]

# ╔═╡ b61bca3f-adc8-41ce-8705-31c632f1fcce
y = mcycle[:,:Accel]

# ╔═╡ 8f6ccb57-f3b6-4584-9a02-ad38ad0bdb27
fx = f(x, 1.0)

# ╔═╡ f86774eb-de55-4dcb-8cec-18e23311a53e
logpdf(fx, y)

# ╔═╡ 53725b8f-dbb9-48a4-abfe-e8e4377de5f8
p_fx = posterior(fx, y)

# ╔═╡ 10811e93-307c-4851-8614-d729f5ec737a
let
	fig = Figure()
	ax = Axis(fig[1,1]; xlabel="Time", ylabel="Accel")
	xtest = LinRange(minimum(x), maximum(x), 500)
	μ, σ2 = mean_and_var(p_fx(xtest))
	σ = sqrt.(σ2)
	scatter!(ax, x, y)
	lines!(ax, xtest, μ)
...
# ╔═╡ cc394385-4e26-4278-aa44-68fed5051e3d
raw_init_params = (
	mean = 0.0,
	noise_var = positive(1.0),
	k = (
		var = positive(1.0),
		precision = positive(1/60)
	)
)

# ╔═╡ 06983fff-3563-4c54-888a-1af85b7bece0
ParameterHandling.value(raw_init_params)

# ╔═╡ 428dca41-322c-4c0b-92cf-b28b60f4858a
initθ, unflatten = ParameterHandling.value_flatten(raw_init_params)

# ╔═╡ a3872f06-7f37-4435-b19c-42e2b0b8c75c
unflatten(initθ)

# ╔═╡ 1eefd3db-02e4-48c0-805b-5f5853255a4d
function gp(params)
	kernel = params.k.var * (Matern52Kernel() ∘ ScaleTransform(params.k.precision))
	return GP(params.mean, kernel)
end

# ╔═╡ e2415c2c-c364-4afc-ad09-6077d52e04eb
function nlml(θ)
	params = unflatten(θ)
	f = gp(params)
...
# ╔═╡ 76a4c356-b3f7-4ba1-ad21-81cab8cede68
nlml(initθ)

# ╔═╡ ae2eda6b-34ea-4701-b1ef-48bf38f68971
Zygote.gradient(nlml, initθ)

# ╔═╡ cdfb2279-7886-4118-ae2b-51a547f4e459
function nlml_fg!(F, G, θ)
	if G !== nothing
		val, grad = Zygote.withgradient(nlml, θ)
		copyto!(G, only(grad))
		if F !== nothing
			return val
		end
...
# ╔═╡ 30c550d5-b85a-4f38-b77b-eb9c325c26c6
res = Optim.optimize(
	Optim.only_fg!(nlml_fg!),
	initθ,
	Optim.LBFGS(
...
# ╔═╡ 3d4fbce2-126c-45a0-8c12-0bf460e18ad8
optθ = Optim.minimizer(res)

# ╔═╡ bfe11ab9-d3c7-40f7-9f68-f6d2b35f6289
opt_params = unflatten(optθ)

# ╔═╡ 837995e4-2ad1-4bde-bc94-dcb7b4e55fd3
opt_f = gp(opt_params)

# ╔═╡ 9d27707d-f74e-4d91-bff1-b5be068a079d
opt_p_fx = posterior(opt_f(x, opt_params.noise_var), y)

# ╔═╡ 18ded560-1cd2-4bbc-8513-cec1743916bc
let
	fig = Figure()
	ax = Axis(fig[1,1]; xlabel="Time", ylabel="Accel")
	xtest = LinRange(minimum(x), maximum(x), 500)
	μ, σ2 = mean_and_var(opt_p_fx(xtest))
	σ = sqrt.(σ2)
	scatter!(ax, x, y)
	lines!(ax, xtest, μ)
	band!(ax, xtest, μ .- (2 * σ), μ .+ (2 * σ); color=(:grey, 0.5))
...
# ╔═╡ 6e7d47ae-45dd-4df5-815c-dbd1cc519192
@benchmark Optim.optimize(
	Optim.only_fg!(nlml_fg!),
	initθ,
	Optim.LBFGS(
		alphaguess = LineSearches.InitialStatic(scaled=true),
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "c322b5ca275d240540f6d73e025c84e2839cbed7"
...
git-tree-sha1 = "bd32cf623e18935a94ff10961ca02c4062fe1dbe"
